...
impl<'a, 'b> Parser<'a, 'b> {
    ...
    fn new_block(&self) -> Block<'a> {
        Block {
            ...
            labels: vec![],
            aligned: false,
            ty: if self.section == Section::Code {
                BlockType::Code(vec![])
            } else {
                BlockType::Data(vec![])
            },
    ...
    fn get_directive_arg(&mut self, directive_type: DirectiveType) -> Result<Token<'a>, String> {
        ...
        match next {
            Some(n) => Ok(n),
            None => Err(directive_error(directive_type, None)),
        }
    }
            ...
            match next.ty {
                ...
                TokenType::Directive(ty @ (DirectiveType::Const | DirectiveType::Uconst)) => {
                    ...
                    let val = match num.ty {
                        TokenType::Literal(LiteralType::Signed(val))
                            if ty == DirectiveType::Const =>
                        {
                            val as i32
                        }
                        TokenType::Literal(LiteralType::Unsigned(val))
                            if ty == DirectiveType::Uconst =>
                        {
                            val as i32
                        }
                        TokenType::Identifier(Identifier::Hex(val))
                            if ty == DirectiveType::Uconst =>
                        {
                            val as i32
                        }
                    ...
                    if let Some(old) = self.constants.insert(label.chars, val) {
                        return Err(format!(
                            "Label '{}' is already associated with value '{}'",
                            label.chars, old
                        ));
                    }
                }
    ...
    fn parse_instruction(
        ...
        ty: InstructionType,
        specs: &[Operand],
    ) -> Result<InstructionWithLabel<'a>, String> {
        let mut instruction = InstructionWithLabel {
            ty,
            rd: -1,
            rt: -1,
            rs: -1,
        ...
        for (i, &spec) in specs.iter().enumerate() {
            ...
            let token = if let Some(t) = self.consume() {
                ...
                return Err(format!(
                    "Instruction '{}' expects a {} as its {} operand, but found end of file.",
                    ty,
                    op_err_str(spec),
                    number(i + 1)
                ));
        ...
        fn error(
            ...
            format!(
                "Instruction '{}' expects a {} as its {} operand, but found '{}'.",
                instruction.ty,
                op_err_str(spec),
                number(i + 1),
                chars
        ...
        fn immediate(
            ...
            signed: bool,
            bits: u8,
            instruction: &mut InstructionWithLabel,
        ) -> Result<(), String> {
            ...
            if !number_fits(lit_value, signed, bits) {
                let signedness = if signed { "a signed" } else { "an unsigned" };
    ...
    fn parse_instructions(
        &mut self,
        instructions: &mut Vec<InstructionWithLabel<'a>>,
    ) -> Result<(), String> {
    ...
    fn next_block(&mut self) -> Option<Result<Block<'a>, String>> {
        ...
        if block.is_empty() {
            Some(Err("Empty block!".to_string()))
        } else {
            Some(Ok(block))
        }
    }
...
fn directive_error(directive: DirectiveType, found: Option<&Token>) -> String {
    ...
    match found {
        Some(f) => format!(
            "Expected {} after .{} directive, but found '{}'.",
            es, ds, f.chars
        ),
        None => format!(
            "Expected {} after .{} directive, but found end of file.",
            es, ds
        ),
    }
}
