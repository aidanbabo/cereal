...
TRAP_PUTS
	...
	ADD R3, R0, #0     ; Copy Pointer

TRAP_PUTS_LOOP
	LDR R0, R3, #0     ; Load Value
	Brz TRAP_PUTS_END  ; End if 0

;; Print character
TRAP_PUTS_CHARACTER_LOOP
	LC R4, OS_ADSR_ADDR
	LDR R1, R4, #0
	BRzp TRAP_PUTS_CHARACTER_LOOP ; Loop while the MSB is zero

	LC R4, OS_ADDR_ADDR
	STR R0, R4, #0		; Write out the character

	ADD, R3, R3, #1    ; Next Memory Location
...
TRAP_DRAW_RECT_OUTER

	CMPIU R3, #0					; If height is 0 end
	Brz TRAP_DRAW_RECT_END
	ADD R5, R0, #0					; column counter
	LC R6, OS_VIDEO_NUM_COLS		; number of columns
	MUL R6, R6, R1					; get starting row

TRAP_DRAW_RECT_LOOP
	ADD R7, R0, R2					; r6 = end column
	CMP R5, R7						; if column counter is end column, go to next row
	Brz TRAP_DRAW_RECT_RECURSION
	LEA R7, OS_VIDEO_MEM			; pointer to start of vmem
	ADD R7, R7, R6					; add starting row
	ADD R7, R7, R5					; add column
	STR R4, R7, #0					; store color at address
	ADD R5, R5, #1					; add one to column counter
	Brnzp TRAP_DRAW_RECT_LOOP

TRAP_DRAW_RECT_RECURSION
	ADD R1, R1, #1					; increment row
	ADD R3, R3, #-1					; decrement height 
	BRnzp TRAP_DRAW_RECT_OUTER
